...
if __name__ == "__main__":
    ...
    for img_file in img_files[0:]:
        ...
        bbs, prob = digit_spotter.run(img, threshold=0.5, do_nms=True,show_result=True, nms_threshold=0.05)
        print (bbs)
        print(prob)
        for bb in bbs:
            y1, y2, x1, x2 = bb
            cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 225), 2)
        if cv2.imwrite("outputs/" + str(i) +".jpg",img):
            print("outputs/" + str(i) +".jpg" + "   written!")
        else:
            print("outputs/" + str(i) +".jpg" + "   wrong!")
        i = i + 1
