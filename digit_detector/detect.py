...
class DigitSpotter:
    ...
    def __init__(self, classifier1, classifier2, classifier3, recognizer, region_proposer):
        """
        Parameters:
            model_file (str)
            region_proposer (MserRegionProposer)
        """
        self._cls1 = classifier1
        self._cls2 = classifier2
        self._cls3 = classifier3
    ...
    def run(self, image, threshold=0.7, do_nms=True, show_result=True, nms_threshold=0.3):
        ...
        patches = candidate_regions.get_patches(dst_size=self._cls1.input_shape)
        ...
        if len(patches) < 4:
            return None
        probs = self._cls1.predict_proba(patches)[:, 1]
        ...
        if len(patches) < 4:
            return None
        probs = self._cls2.predict_proba(patches)[:, 1]
    
        # 4. Thresholding
        bbs, patches, probs = self._get_thresholded_boxes(bbs, patches, probs, threshold)
        ...
        if len(patches) < 4:
            return None
        probs = self._cls3.predict_proba(patches)[:, 1]
        ...
        if show_result:
            ...
            for i, bb in enumerate(bbs):
                ...
                output = output + [(bb,probs[i],msg)]
            #plt.show("MSER + CNN", image)
            cv2.waitKey(0)
